...
namespace ConsoleApp1
{
    public abstract class TaskRunnerBase
    {
            ...
            internal void MonitorWorker(string path, ManualResetEvent readyRunWait, CancellationTokenSource cancel)
            {
                ...
                using (TextWriter output = new StreamWriter(path, false, Encoding.ASCII))
                {
                    ...
                    do
                    {
                        ...
                        //int[] x = null;
                        lock (this._syncroot)
                        {//x = this._thread_task_step_map.Keys.ToArray();

                            foreach (int mtid in this._thread_last_step_map.Keys)
                            {
                            ...
                            output.Write($",{this._thread_last_step_map.Count}");
                        ...
                        readyRunWait.Set();
                        SpinWait.SpinUntil(() => {
                            return this._execution_timer.ElapsedMilliseconds >= count * monitor_sample_time;
                        });
        ...
        public void ExecuteTasks(int taskCount = 1000)
        {
            ...
            string execute_log_filepath = $"execute-log-{DateTime.Now:yyyy-MMdd}-{taskCount}.csv";
